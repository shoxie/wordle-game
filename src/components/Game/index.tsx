...
export default function GamePanel() {
  ...
  const getStatuses = () => {
    ...
    for (let i = 0; i < currentRow.length; i++) {
      if (!solution.includes(text[i].toLocaleLowerCase())) currentRow[i].status = "absent";
      if (solution.includes(text[i].toLocaleLowerCase())) currentRow[i].status = "present";
      if (solution[i] === text[i].toLocaleLowerCase()) currentRow[i].status = "correct";
    }
    setRows([...rows]);
  }
  ...
  return (
    ...
    <VStack justify="center" h="full" spacing={20}>
      ...
      <Keyboard onLetterClick={handleLetterClick} onSubmit={handleSubmit} rows={rows.slice(0, currentRowIndex)} onDelete={deleteChar} />
    </VStack>
